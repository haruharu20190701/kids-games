...
├── games/<slug>/index.html 1ゲーム＝1フォルダ＝1ファイル完結（現在 25本）
...
4. ルート `index.html` の `GAMES` 配列に登録: `{ name, emoji, img:"icons/<slug>.png", path }`。
5. **`sitemap.xml` に新ページの `<url>` を1ブロック追記**（メニュー登録と sitemap は必ず一致させる）。
6. **メニューアイコンを生成**（`kind=icon`・下記ハンズフリー起動）して `img` に設定する。**標準手順＝ユーザーに毎回きかない**。読み込み失敗時は emoji にフォールバック。
7. テスト → コミット → プッシュ。
- スキル `/new-kids-game` で手順を呼び出せる。
- ⚠️ メニュー登録・sitemap・head の SEO は3点セット。どれか1つだけ漏れがちなので追加時に必ず揃える。**＋メニューアイコンも必須**（全ゲーム アイコン画像で統一する方針）。
- 🖼️ **画像は積極的に使う**：ゲームの見た目・楽しさが良くなるなら、指示が無くても 生成画像（キャラ・背景・素材）を使う判断をする。ただし **emoji や canvas図形のフォールバックを必ず用意**し、画像未到着でも遊べる状態で公開する。生成は「★Claude向け実行手順」のハンズフリー起動（`imagegen-requests/request.txt` を push）で行い、**生成中は main へ別 push しない**（push競合回避）。
